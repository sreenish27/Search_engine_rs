...
pub fn read_postings(term: &str, term_index: &HashMap<String, TermEntry>, tier_idx: usize) -> Option<HashMap<u32, Vec<u32>>> {
    let meta = term_index.get(term)?;
    let mut file = File::open("final_index.bin").unwrap();
    file.seek(SeekFrom::Start(meta.tiers[tier_idx].0)).unwrap();
    let mut buffer = vec![0u8; meta.tiers[tier_idx].1 as usize];
